...
fetch('https://raw.githubusercontent.com/DigitalArsenal/SensorShadow/main/src/SensorShadow.fragment.shader.glsl')
  .then(async(data)=>{
  const fsShader = (await data.text()).replace("export default `", "").replace("`;","");
...
let centralParkPosition = Cesium.Cartesian3.fromDegrees(-73.9654, 40.7829, 100); // Central Park

let empireStateBuildingPosition = Cesium.Cartesian3.fromDegrees(
    -73.9857,
    40.748817,
    10
...
const viewer = new Cesium.Viewer("cesiumContainer");
 viewer.camera.moveEnd.addEventListener(function () {
    ...
    let savedCameraStateJSON = localStorage.getItem('cameraState');
    savedCameraStateJSON = savedCameraStateJSON ? JSON.parse(savedCameraStateJSON) :
        {
            "position":
                { "x": 1336417.618721401, "y": -4647708.593285699, "z": 4145763.88177736 },
            "direction": { "x": -0.31230587822311345, "y": -0.4553596230372403, "z": -0.8337341615495147 },
            "up": { "x": 0.12714789222823586, "y": -0.8897716297659011, "z": 0.4383376100286731 }
        };

    if (savedCameraStateJSON) {
        const savedCameraState = savedCameraStateJSON;
        ...
        viewer.camera.direction = new Cesium.Cartesian3(
            savedCameraState.direction.x,
            savedCameraState.direction.y,
            savedCameraState.direction.z);
        viewer.camera.up = new Cesium.Cartesian3(
            savedCameraState.up.x,
            savedCameraState.up.y,
            savedCameraState.up.z);
    }

    viewer.clock.shouldAnimate = false;
    //return;
    const redBall = viewer.entities.add({
        position: centralParkPosition,
        point: {
            pixelSize: 10,
    ...
    var sensorShadowInstance = new SensorShadow(viewer, {
        ...
        viewPosition: empireStateBuildingPosition,
    });
